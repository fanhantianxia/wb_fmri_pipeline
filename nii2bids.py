...
class Data2Bids():
    ...
    def __init__(self, input_dir="/data", config=None, output_dir=None):
    ...
    #function name : match_anat_or_func
    #function goal : 1.Determine whether the input matches the content?
    #                2.if matched, output the result  
    def match_anat_or_func(self, config_regexp, filename):
        delimiter_left = config_regexp["left"]
    ...
    def match_FuncTask(self, config_regexp, filename):
        delimiter_left = config_regexp["left"]
        delimiter_right = config_regexp["right"]
        ...
        if (self._data_dir is not None
            ...
            for root, _, files in os.walk(self._data_dir):
                for file in files:
                    ...
                    '''
                    try:
                        part_match = self.match_PartNum(self._config["PartNum"], file)
                        dst_file_path = dst_file_path + part_match
                        new_name = new_name + part_match
                    except AssertionError:
                        print("No participant num found for %s" %src_file_path)
                        continue
                    '''
                    # Matching the anat/fmri data type and task
                    try:
                        data_type_match = self.match_anat_or_func(self._config["anat"]
                                                            , file)
                        dst_file_path = dst_file_path + "/anat"
                    except AssertionError:
                        # If no anatomical, trying functionnal
                        try:
                            data_type_match = self.match_anat_or_func(self._config["func"]
                                                                , file)
                            dst_file_path = dst_file_path + "/func"
                            # Now trying to match the task
                            try:
                                task_label_match = self.match_FuncTask(self._config["func.task"]
                                                                     , file)
